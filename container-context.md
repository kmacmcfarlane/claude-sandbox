...
- **Memory is capped, with swap off.** `$CLAUDE_SANDBOX_MEMORY_LIMIT` is this
  ...
  the host's preferred OOM victims (`oom_score_adj` 500 by default, the
  `oomScoreAdj` key; see `/proc/self/oom_score_adj`): when the HOST runs out
...
- **Docker bind mounts resolve on the host.** The Docker CLI talks to the host
  ...
  works it refuses with exit 2 — set `TMPDIR` to the scratchpad and retry.
