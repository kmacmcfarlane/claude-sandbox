...
var _ = Describe("ralph scaffold seeding filter", func() {
	It("CS-INITR-007: __pycache__, *.pyc, .pytest_cache and dot-entries are never seeded", func() {
		...
		fsys := fstest.MapFS{
			...
			"scaffold-ralph/scripts/backlog/X.PYC":                         f("pyc"),
			"scaffold-ralph/scripts/backlog/foo.pyc/inner.txt":             f("inside"),
		...
		for _, rel := range []string{
			...
			"scripts/backlog/X.PYC",
			"scripts/backlog/foo.pyc",
			"scripts/backlog/foo.pyc/inner.txt",
