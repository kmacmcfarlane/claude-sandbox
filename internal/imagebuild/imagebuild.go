...
// build context (CS-IMG-010..014).
func ResolveChild(in ChildInputs, out io.Writer) ChildSpec {
	spec := ChildSpec{ImageName: BaseImageName + "-" + in.Slug}
	if in.BaseOnly {
		return spec
	}
	...
	if override {
		...
		if fileExists(df) {
			...
			return ChildSpec{Use: true, Dockerfile: df, Context: dir, ImageName: spec.ImageName}
		}
		...
		if found := paths.FindUpFile(filepath.Dir(dir), name); found != "" {
			...
			return ChildSpec{Use: true, Dockerfile: found, Context: filepath.Dir(found), ImageName: spec.ImageName}
		}
		spec.Dockerfile = df
		return spec
	}
	...
	if fileExists(df) {
		return ChildSpec{Use: true, Dockerfile: df, Context: in.ProjectDir, ImageName: spec.ImageName}
	}
	if found, _ := paths.FindUp(filepath.Dir(in.ProjectDir), paths.Dockerfile); found != "" {
		...
		return ChildSpec{Use: true, Dockerfile: found, Context: ctx, ImageName: spec.ImageName}
	}
	spec.Dockerfile = df
	return spec
}
...
// lowercased, characters outside [a-z0-9._-] replaced with '-'.
