...
Attach: `claude-sandbox --attach=otter` · container `claude-sandbox-work-claude-sandbox-a1b2c3-otter`
```

The bold name is the session's name as `/peers` shows it — read from that session's own peer-registry record (`<config dir>/sessions/$CLAUDE_PID.json`, only when its `sessionId` matches), never from the transcript. The noun, container and mode come from `CLAUDE_SANDBOX_INSTANCE`, `CLAUDE_SANDBOX_CONTAINER` and `CLAUDE_SANDBOX_MODE`, which the launcher sets in every container. An idle ping reads `🔔 idle · …` and quotes no message; only a permission prompt quotes Claude Code's one-line message, which names the tool. A joined session says `Joined session (not attachable)` instead of offering `--attach` (attach reaches the container's primary session; joins set `CLAUDE_SANDBOX_JOINED=1`), a headless one says `Headless session (SDK client)`, and ralph (no noun) shows the container alone. The payload never carries an env value, a token, the webhook URL, a path beyond the project's basename or any conversation text; the body is built with `jq` and sets `allowed_mentions.parse: []`, so no name can ping `@everyone`. A missing field is dropped, and when nothing identifies the session it posts the old single line, `🔔 Claude Code needs your input`. The script never fails a session (it always exits 0), makes no docker call, and sends one `curl` with a 10 s cap.
