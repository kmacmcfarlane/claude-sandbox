...
Claude Code merges hook entries across settings levels, so these run **alongside** any hooks in your own `~/.claude/settings.json`; they do not replace them. They appear as managed ("organization") settings in `/status`, and a user-level `disableAllHooks` does not turn them off. A child image can add its own `/etc/claude-code/managed-settings.json` or another drop-in without removing them.
