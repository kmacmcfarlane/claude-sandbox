...
- **Parent directory search:** the config, Dockerfile, and env files (under `.claude-sandbox/`) are all resolved by walking parent directories from the project root (direnv-style). This lets a monorepo or workspace root provide shared config for all sub-projects.
...
bin/lib/paths.sh     # Foreign-path resolver — single source of .claude-sandbox/ path mapping + layout lifecycle
