...
- **Detach keys:** default `ctrl-q,ctrl-q`, overridable via `detachKeys`. Docker's own default `ctrl-p,ctrl-q` is unusable here — the Claude Code TUI binds `ctrl+p`, along with nearly every other ctrl letter and `ctrl+]`/`ctrl+\`/`ctrl+_`; `ctrl-q`, `ctrl-^`, `ctrl-@` are the unbound ones.
